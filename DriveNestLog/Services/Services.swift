...
protocol StorageService {
    ...
    func savePermissions(_ permission: PermissionState)
    func markLaunched()
...
final class HTTPNetworkService: NetworkService {
    ...
    func fetchAttribution(deviceID: String) async throws -> [String: Any] {
        var builder = URLComponents(string: "https://gcdsdk.appsflyer.com/install_data/v4.0/id\(DriveNestConfig.appID)")
        builder?.queryItems = [
            URLQueryItem(name: "devkey", value: DriveNestConfig.devKey),
    ...
    func fetchEndpoint(tracking: [String: Any]) async throws -> String {
        guard let url = URL(string: "https://drivenestlog.com/config.php") else {
            throw NetworkError.invalidURL
        }
        ...
        payload["store_id"] = "id\(DriveNestConfig.appID)"
        ...
        var lastError: Error?
        let retries: [Double] = [21.0, 42.0, 84.0]
...
struct DriveNestConfig {
    static let appID = "6761115476"
    static let devKey = "Z7ahpVzau9eLuKKPU7KKPJ"
}
